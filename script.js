...
function showLightboxImage() {
    ...
    const altText = currentImages[currentIndex].alt || '';
    const captionText = currentImages[currentIndex].closest('.gallery__item')?.querySelector('.gallery__caption')?.textContent || altText;
...
// Обычные фото в новостях (вне слайдера)
...
document.querySelectorAll('.section, .footer, .news-card').forEach(el => {
    observer.observe(el);
});
