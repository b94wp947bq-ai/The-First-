...
// Анимация секций и новостных карточек
...
// Наблюдаем за секциями, футером и карточками новостей
document.querySelectorAll('.section, .footer, .news-card').forEach(el => observer.observe(el));

// Hero сразу видимый
const hero = document.querySelector('.hero');
if (hero) hero.classList.add('visible');

// На странице новостей первый блок сразу видим
if (!hero) {
    const firstCard = document.querySelector('.news-card');
    if (firstCard) {
        firstCard.classList.add('visible'); // добавляем класс visible
    }
}