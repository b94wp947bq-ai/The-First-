...
// Кнопка "Читать далее" для длинных новостей
document.querySelectorAll('.news-card__text').forEach(textBlock => {
    const fullText = textBlock.textContent.trim();
    const maxLength = 400;

    if (fullText.length > maxLength) {
        // Обрезаем по последнему пробелу перед maxLength
        let preview = fullText.slice(0, maxLength);
        ...
        textBlock.innerHTML = `
            <span class="news-card__text-preview">${preview}</span>
            <span class="news-card__text-full" style="display: none;">${fullText}</span>
        `;

        const btn = document.createElement('button');
        btn.className = 'news-card__read-more';
        btn.textContent = 'Читать далее';

        btn.addEventListener('click', () => {
            const previewEl = textBlock.querySelector('.news-card__text-preview');
            const fullEl = textBlock.querySelector('.news-card__text-full');
